...
import {withDefaultBase, withFormatting, withScheme} from '../utils/url';
 ...
 * `r`: Rank.
 */
type DuckDuckGoBang = Record<'c' | 'd' | 's' | 'sc' | 't' | 'u', string> & {
    r: number;
};
...
export default class DuckDuckGo extends Backend<DuckDuckGoBang> {
    ...
    formatBang = ({t, d, u}: DuckDuckGoBang): Bang => ({
        ...
        url: withFormatting(
            withScheme(withDefaultBase(u, 'https://duckduckgo.com'), 'https'),
        ),
    });
}
